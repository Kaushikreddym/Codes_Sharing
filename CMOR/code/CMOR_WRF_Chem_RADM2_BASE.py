...
def change_path(op):
    ...
    outpath="proc\/"+op+"\"\,"
    inpath="proc.*$"
    input_file="../cmor/common_user_input.json"
    print("sed -i 's/"+inpath+'/'+outpath+'/g\' '+input_file)
    os.system("sed -i 's/"+inpath+'/'+outpath+'/g\' '+input_file)
change_path(op)
...
for mon_no in ["06","07","08","09"]:
    ...
    if flag_rain == 1:
        ...
        dset_PRSH = xray.concat(dset_PRSH, 'Time')
        # level = np.array(plevs)
